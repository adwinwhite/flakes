#!/nix/store/dndi916j6yxzfzzj2sma2llhrlwahq06-bash-5.1-p16/bin/bash
...
iptables -w 60 -t mangle -A TPROXY_OUT -m mark --mark 0xff -j RETURN
for cg in ${cgroup_noproxy[@]}; do
...
ip6tables -w 60 -t mangle -A TPROXY_OUT -m mark --mark 0xff -j RETURN
for cg in ${cgroup_noproxy[@]}; do
